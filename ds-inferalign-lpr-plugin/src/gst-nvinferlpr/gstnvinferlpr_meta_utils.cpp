...
static gpointer copy_user_meta(gpointer data, gpointer user_data)
{
  ...
  float *src_user_metadata = (float*)user_meta->user_meta_data;
  float *dst_user_metadata = (float*)g_malloc0(10 * sizeof(float));
  memcpy(dst_user_metadata, src_user_metadata, 10 * sizeof(float));
...
void *set_metadata_ptr(const std::vector<std::string>& landmarks, GstNvInferLpr * nvinferlpr, NvDsObjectMeta *parent_obj_meta, GstNvInferLprFrame & frame){
  float *user_metadata = (float*)g_malloc0(10 * sizeof(float));
  for (unsigned int i = 0; i < landmarks.size() - 1; ) {
    if (i + 1 < landmarks.size() && i + 2 < landmarks.size()) {
      try {
        user_metadata[i] = (std::stof(landmarks[i + 1]) - frame.offset_left * 1.0) / frame.scale_ratio_x * 1.0 + frame.roi_left * 1.0;
        user_metadata[i + 1] = (std::stof(landmarks[i + 2]) - frame.offset_top * 1.0) / frame.scale_ratio_y * 1.0 + frame.roi_top * 1.0;

        // user_metadata[i] = std::stof(landmarks[i + 1]);
        // user_metadata[i] = std::stof(landmarks[i + 2]);

        if (!nvinferlpr->process_full_frame) {
      ...
      std::cerr << "Not enough landmarks in the vector." << std::endl;
    }
  }
...
void
attach_metadata_detector (GstNvInferLpr * nvinferlpr, GstMiniObject * tensor_out_object,
    GstNvInferLprFrame & frame, NvDsInferDetectionOutput & detection_output, float segmentationThreshold)
{
  ...
  for (guint i = 0; i < detection_output.numObjects; i++) {
    ...
    GstNvInferLprDetectionFilterParams & filter_params =
        (*nvinferlpr->perClassDetectionFilterParams)[obj.classIndex];
    ...
    if(nvinferlpr->filter_out_class_ids->find(obj.classIndex) != nvinferlpr->filter_out_class_ids->end())
        continue;
    ...
    obj_meta->unique_component_id = nvinferlpr->unique_id;
    ...
    if (obj.classIndex > (gint) nvinferlpr->perClassColorParams->size()) {
      rect_params.has_bg_color = 0;
      rect_params.border_color = (NvOSD_ColorParams) {1, 0, 0, 1};
    } else {
      GstNvInferLprColorParams &color_params = (*nvinferlpr->perClassColorParams)[obj.classIndex];
    ...
    if (obj.label && nvinferlpr->alignment_type == 0){
      g_strlcpy (obj_meta->obj_label, obj.label, MAX_LABEL_SIZE);
      text_params.display_text = g_strdup (obj.label);
    } else if (obj.label && nvinferlpr->alignment_type != 0){
      ...
      user_meta->user_meta_data = (void *)set_metadata_ptr(landmarks, nvinferlpr, parent_obj_meta, frame);
    ...
    if (nvinferlpr->output_instance_mask && obj.mask) {
 ...
 */
void
attach_metadata_classifier (GstNvInferLpr * nvinferlpr, GstMiniObject * tensor_out_object,
    GstNvInferLprFrame & frame, GstNvInferLprObjectInfo & object_info)
{
  NvDsObjectMeta *object_meta = frame.obj_meta;
  NvDsBatchMeta *batch_meta = (nvinferlpr->process_full_frame) ?
  ...
  nvds_acquire_meta_lock (batch_meta);
  if (nvinferlpr->process_full_frame && !nvinferlpr->input_tensor_from_meta) {
  ...
  classifier_meta->unique_component_id = nvinferlpr->unique_id;
  classifier_meta->classifier_type = nvinferlpr->classifier_type;
 ...
 */
void
merge_classification_output (GstNvInferLprObjectHistory & history,
...
void
attach_metadata_segmentation (GstNvInferLpr * nvinferlpr, GstMiniObject * tensor_out_object,
    GstNvInferLprFrame & frame, NvDsInferSegmentationOutput & segmentation_output)
{
  ...
  meta->unique_id = nvinferlpr->unique_id;
...
void
attach_tensor_output_meta (GstNvInferLpr *nvinferlpr, GstMiniObject * tensor_out_object,
    GstNvInferLprBatch *batch, NvDsInferContextBatchOutput *batch_output)
{
  NvDsBatchMeta *batch_meta = (nvinferlpr->process_full_frame
      || nvinferlpr->input_tensor_from_meta) ? batch->frames[0].
   ...
   * increment the refcount of GstNvInferLprTensorOutputObject. */
  for (size_t j = 0; j < batch->frames.size (); j++) {
    GstNvInferLprFrame & frame = batch->frames[j];
    ...
    meta->unique_id = nvinferlpr->unique_id;
    meta->num_output_layers = nvinferlpr->output_layers_info->size ();
    meta->output_layers_info = (NvDsInferLayerInfo*)g_memdup(nvinferlpr->output_layers_info->data (),
    ...
    meta->gpu_id = nvinferlpr->gpu_id;
    meta->priv_data = gst_mini_object_ref (tensor_out_object);
    meta->network_info = nvinferlpr->network_info;
    meta->maintain_aspect_ratio = nvinferlpr->maintain_aspect_ratio;
