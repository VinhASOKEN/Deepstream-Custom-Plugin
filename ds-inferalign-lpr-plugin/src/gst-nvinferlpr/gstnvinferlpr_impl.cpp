...
GST_DEBUG_CATEGORY_EXTERN (gst_nvinferlpr_debug);
...
DsNvInferImpl::DsNvInferImpl (GstNvInferLpr * infer)
    ...
    const NvDsInferContextInitParams & oldParams)
{
  ...
  switch (loadType) {
    case MODEL_LOAD_FROM_CONFIG:
      if (!newModelPath.compare (newModelPath.length() - 4, 4, string_txt)) {
        if (!gst_nvinferlpr_parse_context_params (&newParams, newModelPath.c_str ())) {
    ...
    NvDsInferContextInitParams & newParam)
{
  if (newParam.maxBatchSize < m_GstInfer->max_batch_size) {
    GST_WARNING_OBJECT (m_GstInfer,
        "[UID %d]: New model batch-size[in config] (%d) is smaller then gst-nvinferlpr's"
  ...
  if (m_GstInfer->network_width != (gint) networkInfo.width ||
    ...
    GST_WARNING_OBJECT (m_GstInfer,
        ...
        "gst-nvinferlpr's current resolution (%dx%d).", m_GstInfer->unique_id,
...
DsNvInferImpl::notifyLoadModelStatus (const ModelStatus & res)
{
  ...
  g_signal_emit (m_GstInfer, gst_nvinferlpr_signals[SIGNAL_MODEL_UPDATED], 0,
...
/* Wait till all the buffers with gst-nvinferlpr are inferred/post-processed and
 * pushed downstream. */
NvDsInferStatus
    ...
    NvDsInferContextInitParamsPtr params, const std::string & path)
{
  ...
  GstNvInferLpr *nvinferlpr = m_GstInfer;
  g_free (nvinferlpr->config_file_path);
  nvinferlpr->config_file_path = g_strdup (path.c_str ());
  /* Get the network resolution. */
  ctx->getNetworkInfo (nvinferlpr->network_info);
  ...
  ctx->fillLayersInfo (*nvinferlpr->layers_info);

  nvinferlpr->output_layers_info->clear ();
