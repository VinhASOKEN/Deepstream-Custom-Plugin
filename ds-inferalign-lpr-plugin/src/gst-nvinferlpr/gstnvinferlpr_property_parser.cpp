...
static gboolean
gst_nvinferlpr_parse_class_attrs (GKeyFile * key_file, gchar * group,
    NvDsInferDetectionParams & detection_params,
    GstNvInferLprDetectionFilterParams & detection_filter_params,
    GstNvInferLprColorParams & color_params)
{
  gboolean ret = FALSE;
  gchar **keys = nullptr;
  gchar **key = nullptr;
...
static gboolean
gst_nvinferlpr_parse_other_attribute (GstNvInferLpr * nvinferlpr,
    ...
    const gchar * cfg_file_path)
{
  ...
  if (!g_strcmp0 (key, CONFIG_GROUP_INFER_PROCESS_MODE)) {
    if ((*nvinferlpr->is_prop_set)[PROP_PROCESS_MODE])
      return TRUE;
    ...
    switch (val) {
      case 1:
        nvinferlpr->process_full_frame = TRUE;
        break;
      case 2:
        nvinferlpr->process_full_frame = FALSE;
        break;
      default:
    ...
    nvinferlpr->min_input_object_height = g_key_file_get_integer (key_file,
        group_name, CONFIG_GROUP_INFER_INPUT_OBJECT_MIN_HEIGHT,
        &error);
    CHECK_ERROR (error);
    if ((gint) nvinferlpr->min_input_object_height < 0) {
      g_printerr ("Error: Negative value specified for %s(%d)\n",
    ...
    nvinferlpr->max_input_object_width = g_key_file_get_integer (key_file,
        group_name, CONFIG_GROUP_INFER_INPUT_OBJECT_MAX_WIDTH,
        &error);
    CHECK_ERROR (error);
    if ((gint) nvinferlpr->max_input_object_width < 0) {
      g_printerr ("Error: Negative value specified for %s(%d)\n",
          CONFIG_GROUP_INFER_INPUT_OBJECT_MAX_WIDTH,
          nvinferlpr->max_input_object_width);
      goto done;
    }
  } else if (!g_strcmp0 (key, CONFIG_GROUP_INFER_INPUT_OBJECT_MAX_HEIGHT)) {
    nvinferlpr->max_input_object_height = g_key_file_get_integer (key_file,
        group_name, CONFIG_GROUP_INFER_INPUT_OBJECT_MAX_HEIGHT,
        &error);
    CHECK_ERROR (error);
    if ((gint) nvinferlpr->max_input_object_height < 0) {
      g_printerr ("Error: Negative value specified for %s(%d)\n",
          CONFIG_GROUP_INFER_INPUT_OBJECT_MAX_HEIGHT,
          nvinferlpr->max_input_object_height);
      goto done;
    }
  } else if (!g_strcmp0 (key, CONFIG_GROUP_INFER_GIE_ID_FOR_OPERATION)) {
    if ((*nvinferlpr->is_prop_set)[PROP_OPERATE_ON_GIE_ID] ||
        (*nvinferlpr->is_prop_set)[PROP_OPERATE_ON_CLASS_IDS])
      return TRUE;
    nvinferlpr->operate_on_gie_id = g_key_file_get_integer (key_file,
    ...
    for (i = 0; i < length; i++) {
      nvinferlpr->operate_on_class_ids->at (int_list[i]) = TRUE;
    }
    g_free (int_list);
    ...
    for (guint i = 0; i < length; i++) {
      nvinferlpr->filter_out_class_ids->insert(int_list[i]);
    }
    g_free(int_list);
    ...
    nvinferlpr->transform_config_params.compute_mode = (NvBufSurfTransform_Compute) val;
    ...
    nvinferlpr->transform_params.transform_filter = (NvBufSurfTransform_Inter) val;
    ...
    if (g_key_file_get_boolean (key_file, group_name,
      ...
      nvinferlpr->crop_objects_to_roi_boundary = TRUE;
    CHECK_ERROR (error);
  }else if (!g_strcmp0 (key, CONFIG_GROUP_INFER_INPUT_OBJECT_ALIGNMENT_TYPE)) {
    nvinferlpr->alignment_type = g_key_file_get_integer (key_file,
...
static gboolean
gst_nvinferlpr_parse_props (GstNvInferLpr * nvinferlpr,
    ...
    GKeyFile * key_file, const gchar * cfg_file_path)
{
  ...
  for (key = keys; *key; key++) {
    if (!g_strcmp0 (*key, CONFIG_GROUP_INFER_UNIQUE_ID)) {
      if (nvinferlpr && (*nvinferlpr->is_prop_set)[PROP_UNIQUE_ID])
        continue;
      ...
      if (init_params->uniqueID <= 0) {
        g_printerr ("Error: %s (%d) should be > 0\n",
            CONFIG_GROUP_INFER_UNIQUE_ID, nvinferlpr->unique_id);
        goto done;
      }
      if (nvinferlpr)
        nvinferlpr->unique_id = init_params->uniqueID;
      ...
      if (nvinferlpr && (*nvinferlpr->is_prop_set)[PROP_GPU_DEVICE_ID])
        continue;
      gint devices;
      ...
      if (nvinferlpr)
        nvinferlpr->gpu_id = init_params->gpuID;
      ...
      if (nvinferlpr && (*nvinferlpr->is_prop_set)[PROP_BATCH_SIZE])
        continue;
      ...
      if (init_params->maxBatchSize <= 0
        ...
        g_printerr ("Error: %s(%d) should be in the range [%d,%d]\n",
            CONFIG_GROUP_INFER_BATCH_SIZE, nvinferlpr->max_batch_size, 1,
            NVDSINFER_MAX_BATCH_SIZE);
        goto done;
      }
      if (nvinferlpr)
        nvinferlpr->max_batch_size = init_params->maxBatchSize;
    ...
    CHECK_ERROR (error);
      ...
      if (nvinferlpr && (*nvinferlpr->is_prop_set)[PROP_MODEL_ENGINEFILE])
        continue;
      gchar *str = g_key_file_get_string (key_file, CONFIG_GROUP_PROPERTY,
    ...
    switch (val) {
      ...
      if ((*nvinferlpr->is_prop_set)[PROP_INPUT_TENSOR_META])
        continue;
      if (g_key_file_get_boolean (key_file, CONFIG_GROUP_PROPERTY,
              CONFIG_GROUP_INFER_INPUT_FROM_META, &error)) {
        nvinferlpr->input_tensor_from_meta = TRUE;
        init_params->inputFromPreprocessedTensor = TRUE;
      }
        CHECK_ERROR (error);
    } else if (nvinferlpr) {
      if (!gst_nvinferlpr_parse_other_attribute (
          nvinferlpr, key_file, CONFIG_GROUP_PROPERTY, *key, cfg_file_path)) {
        goto done;
      }
    }
  }
...
/* Parse the nvinferlpr config file. Returns FALSE in case of an error. */
gboolean
gst_nvinferlpr_parse_config_file (
    GstNvInferLpr * nvinferlpr,
    NvDsInferContextInitParams *init_params,
    const gchar * cfg_file_path)
{
  GError *error = nullptr;
  ...
  if (!gst_nvinferlpr_parse_props (nvinferlpr, init_params, cfg_file, cfg_file_path)) {
    g_printerr ("Failed to parse group %s\n", CONFIG_GROUP_PROPERTY);
    goto done;
  }
  g_key_file_remove_group (cfg_file, CONFIG_GROUP_PROPERTY, nullptr);

  /* If the nvinferlpr instance is to be configured as a detector, parse the
  ...
  if (init_params->networkType == NvDsInferNetworkType_Detector ||
    ...
    GstNvInferLprDetectionFilterParams detection_filter_params{0, 0, 0, 0, 0, 0};
    GstNvInferLprColorParams color_params;
    ...
    if (g_key_file_has_group (cfg_file,
      ...
      if (!gst_nvinferlpr_parse_class_attrs (cfg_file,
    ...
    nvinferlpr->perClassDetectionFilterParams =
        new std::vector < GstNvInferLprDetectionFilterParams >
        (init_params->numDetectedClasses, detection_filter_params);
    nvinferlpr->perClassColorParams =
        new std::vector < GstNvInferLprColorParams >
    ...
    for (group = groups; *group; group++) {
      if (!strncmp (*group, CONFIG_GROUP_INFER_CLASS_ATTRS_PREFIX,
        ...
        /* Parse the group. */
        if (!gst_nvinferlpr_parse_class_attrs (cfg_file, *group,
                init_params->perClassDetectionParams[class_index],
                (*nvinferlpr->perClassDetectionFilterParams)[class_index],
                (*nvinferlpr->perClassColorParams)[class_index])) {
          g_printerr ("Error while parsing group %s\n", *group);
          goto done;
        }
      }
...
/* Parse nvinferlpr config file for context params. Returns FALSE in case of an error. */
gboolean
gst_nvinferlpr_parse_context_params (
    ...
    const gchar * cfg_file_path)
{
  ...
  if (!gst_nvinferlpr_parse_props (NULL, params, cfg_file, cfg_file_path)) {
    g_printerr ("Failed to parse group %s\n", CONFIG_GROUP_PROPERTY);
    goto done;
  }
  ret = TRUE;
