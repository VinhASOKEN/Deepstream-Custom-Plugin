...
#ifndef __GSTNVINFERLPR_IMPL_H__
#define __GSTNVINFERLPR_IMPL_H__
...
void gst_nvinferlpr_logger(NvDsInferContextHandle handle, unsigned int unique_id,
...
typedef struct _GstNvInferLprObjectHistory GstNvInferLprObjectHistory;
...
typedef struct {
  ...
  std::weak_ptr<GstNvInferLprObjectHistory> history;

} GstNvInferLprFrame;

using GstNvInferLprObjHistory_MetaPair =
    std::pair<std::weak_ptr<GstNvInferLprObjectHistory>, NvDsObjectMeta *>;
...
typedef struct {
   ...
   * Wait and Destroy to be done before preprocess call of nvinferlpr */
  ...
  std::vector <GstNvInferLprObjHistory_MetaPair> objs_pending_meta_attach;
} GstNvInferLprBatch;
 ...
 * is dropped, the callback to free the GstMiniObject-inherited GstNvInferLprTensorOutputObject
...
} GstNvInferLprTensorOutputObject;

namespace gstnvinferlpr {
...
enum ModelLoadType
{
   ...
   * done in gst-nvinferlpr will not be updated. An important requirement is that
...
public:
  ...
  DsNvInferImpl (GstNvInferLpr *infer);
  ~DsNvInferImpl ();
...
private:
  ...
  GstNvInferLpr *m_GstInfer = nullptr;
